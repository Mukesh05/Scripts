...
Function Msgbox($caption,$message,$type,$MaxSize){
    if ($null -eq $MaxSize) { $MaxSize = 125}
...
Function VMSnapshot($vmInfo){
    ...
    ForEach($disk in $vmInfo.DataDisk){
        ...
        $vm_DataDiskSnapConfig = New-AzSnapshotConfig -SourceUri $vm_DataDisk.Id -CreateOption Copy -Location $vmInfo.location -Tag @{"AutoSnap"="True"}
